...
public final class EmptyFieldValidator {
     ...
     * @return true if the field is filled, otherwise false.
     */
    public static synchronized boolean isValid(final ComboBox list, final String message) {

        if (list.getItemIds() != null && list.getItemIds().size() > 0 && list.getValue() != null
            && ((String) list.getValue()).trim().length() > 0) {
            list.setComponentError(null);
            return true;
        }
     ...
     * @return true if the field is filled, otherwise false.
     */
    public static synchronized boolean isValid(final NativeSelect select, final String blank, final String message) {
